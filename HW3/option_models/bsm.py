...
class BsmModel:
    ...
    def delta(self, strike, spot, texp, cp_sign=1):
        d1 = self.cal_d(strike, spot, texp, self.vol)[0]
        if cp_sign == 1:
    ...
    def vega(self, strike, spot, vol, texp, cp_sign=1):
        d1 = self.cal_d(strike, spot, texp, vol)[0]
        vega = spot*np.sqrt(texp)*ss.norm.pdf(d1)
        return vega

    def gamma(self, strike, spot, vol, texp, cp_sign=1):
        d1 = self.cal_d(strike, spot, texp, vol)[0]
        gamma = ss.norm.pdf(d1)/(spot*self.vol*np.sqrt(texp))
        return gamma
