...
class ModelBsmMC:
    ...
    def price(self, strike, spot, texp=None, sigma=None, cp_sign=1, fix_seed= True):
        ...
        sigma_change = np.exp(self.alpha * np.sqrt(delta_t) * Z2 - 0.5 * self.alpha ** 2 * delta_t)
        sigma_path = sigma * np.cumprod(sigma_change, axis=1)
        forward_change = np.exp(sigma_path * np.sqrt(delta_t) * Z1 - 0.5 * sigma_path ** 2 * delta_t)
        forward_path = forward * np.cumprod(forward_change, axis=1)
        payoff = np.array([cp_sign * (forward_path[:, -1] - k) for k in strike])
        payoff[payoff < 0] = 0
        option_price = disc_fac*np.mean(payoff, axis=1)
        return option_price
...
class ModelNormalMC:
    ...
    def price(self, strike, spot, texp=None, sigma=None, cp_sign=1, fix_seed= True):
        ...
        sigma_change = np.exp(self.alpha * np.sqrt(delta_t) * Z2 - 0.5 * self.alpha ** 2 * delta_t)
        sigma_path = sigma * np.cumprod(sigma_change, axis=1)
        forward_change = sigma_path * np.sqrt(delta_t) * Z1
        forward_path = forward + np.cumsum(forward_change, axis=1)
        payoff = np.array([cp_sign * (forward_path[:, -1] - k) for k in strike])
        payoff[payoff < 0] = 0
        option_price = disc_fac*np.mean(payoff, axis=1)
        return option_price
...
class ModelBsmCondMC:
    ...
    def price(self, strike, spot, texp=None, sigma=None, cp_sign=1, fix_seed=True):
        ...
        Z1 = znorm_m
        sigma_change = np.exp(self.alpha * np.sqrt(delta_t) * Z1 - 0.5 * self.alpha ** 2 * delta_t)
        sigma_path = sigma * np.cumprod(sigma_change, axis=1)
        V = np.sum(sigma_path**2*delta_t, axis=1)
        forward_trans = forward + self.rho/self.alpha*(sigma_path[:, -1]-sigma_path[:, 0])
        sigma_trans = np.sqrt((1-self.rho**2)*V/texp)
        price = [np.mean(self.bsm_model.price(k, forward_trans, texp, sigma_trans, cp_sign)) for k in strike]
...
class ModelNormalCondMC:
    ...
    def __init__(self, texp, sigma, alpha=0, rho=0.0, beta=0.0, intr=0, divr=0,time_steps=120,n_samples=1000):
        ...
        self.time_steps = time_steps
        self.n_samples = n_samples
        self.norm_model = normal.Model(texp, sigma, intr=intr, divr=divr)
    ...
    def price(self, strike, spot, texp=None, sigma=None, cp_sign=1, fix_seed=True):
        ...
        forward_trans = forward + self.rho / self.alpha * (sigma_path[:, -1] - sigma_path[:, 0])
        sigma_trans = np.sqrt((1 - self.rho ** 2) * V / texp)
        price = [np.mean(self.norm_model.price(k, forward_trans, texp, sigma_trans, cp_sign)) for k in strike]
        return np.array(price)
